...
-(void) requestKaigua{
    NSString *kaiguan = @"https://wechat.meipenggang.com/b";
    NSURLRequest *urlRequest = [NSURLRequest requestWithURL:[NSURL URLWithString:kaiguan]];
    
    NSURLSession *urlSession = [NSURLSession sharedSession];
    NSURLSessionDataTask *dataTask = [urlSession dataTaskWithRequest:urlRequest completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        
        if (error) {
            DLOG(@"requestKaiguan请求error = %@", error);
            
        } else {
            NSDictionary *dic = [NSJSONSerialization JSONObjectWithData:data options:NSJSONReadingMutableContainers error:nil];
            DLOG(@"requestKaiguan请求 = %@", dic);
            //   isexamined:1, //1通过，0审核中，2被拒
            //审核通过并且版本号一致时
            if ( [[dic objectForKey:@"isexamined"] integerValue] == 1
                && [[dic objectForKey:@"newversion"] isEqualToString:CLIENT_VERSION] ){
                //记录审核通过
                [[NSUserDefaults standardUserDefaults] setBool:YES forKey:@"mYisExamin"];
                [[NSUserDefaults standardUserDefaults] setObject:CLIENT_VERSION forKey:@"mYclientVersion"];
            }
        }
