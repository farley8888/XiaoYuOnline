...
- (void)layoutSubviews {
    ...
    titleFrame.origin.x = ceilf((CGRectGetWidth(self.frame) - _titleLabelWidth) / 2.0);
    titleFrame.origin.y = self.bounds.size.height-(self.subtitleLabel.text ? 48 : 40);
    titleFrame.size.width = _titleLabelWidth;
    self.titleLabel.frame = titleFrame;
    
    CGRect subtitleFrame = self.subtitleLabel.frame;
    subtitleFrame.origin.x = titleFrame.origin.x - 22.0;
    ...
    arrowFrame.origin.x = titleFrame.origin.x - 26.0;
...
- (SVPullToRefreshArrow *)arrow {
    if(!_arrow) {
        ...
        _arrow.arrowColor = [UIColor darkGrayColor];
...
- (UILabel *)titleLabel {
    if(!_titleLabel) {
        _titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, _titleLabelWidth, 20)];
...
- (void)setArrowColor:(UIColor *)newArrowColor {
    ...
    _activityIndicatorView.color = newArrowColor;
...
- (UIColor *)arrowColor {
    ...
    return [UIColor darkGrayColor]; // default Color
}
...
- (void)drawRect:(CGRect)rect {
    ...
    /*
     ...
     CGContextMoveToPoint(c, 11.0, 0);
     CGContextAddLineToPoint(c, 11.0, 30.0);
     */
    CGContextMoveToPoint(c, 11.0, 14.0);
    CGContextAddLineToPoint(c, 11.0, 35.0);
    CGContextAddLineToPoint(c, 6.0, 30.0);
    CGContextMoveToPoint(c, 11.0, 35.0);
    CGContextAddLineToPoint(c, 16.0, 30.0);
    ...
    /*
     // the arrow
     CGContextMoveToPoint(c, 0, 30.0);
     CGContextAddLineToPoint(c, 11, 44.0);
     CGContextAddLineToPoint(c, 22, 30.0);
     CGContextAddLineToPoint(c, 0, 30.0);
     CGContextClosePath(c);
     CGContextSetLineCap(c, kCGLineCapRound);
     CGContextSetLineJoin(c, kCGLineJoinRound);
     CGContextSetFillColorWithColor(c, WKGlobalUI.lightGrayColor.CGColor);
