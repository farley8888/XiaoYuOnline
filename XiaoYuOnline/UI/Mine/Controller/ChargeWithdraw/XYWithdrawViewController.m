...
@interface XYWithdrawViewController ()
...
- (void)viewDidLoad {
    ...
    self.itemList = @[[[XYChargeWithdrawModel alloc] initWithTitle:NSLocalizedString(@"Mine_Setting_Withdraw_BankNum.", nil) content:nil description:bankCard placeHolder:nil],
                      [[XYChargeWithdrawModel alloc] initWithTitle:NSLocalizedString(@"Mine_Setting_Withdraw_Volume", nil) content:nil description:nil placeHolder:NSLocalizedString(@"Mine_Setting_Withdraw_Volume_PH", nil)],
...
- (void)operateButtonAction:(UIButton *)button {
    ...
    for (NSInteger i = 0, count = paramKeys.count; i < count; i ++) {
        XYChargeWithdrawModel *m = (XYChargeWithdrawModel *)self.itemList[i+1];
