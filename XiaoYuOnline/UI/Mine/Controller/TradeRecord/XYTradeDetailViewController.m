...
@interface XYTradeDetailViewController ()
@property (nonatomic, strong) NSArray <XYTradeDetailModel *> *itemList;
@property (nonatomic, assign) BOOL adjustContentInset;
@property (nonatomic, strong) XYTradeRecoderModel *model;
@end

@implementation XYTradeDetailViewController

- (instancetype)initWithTradeModel:(XYTradeRecoderModel *)model {
    self = [super init];
    if (self) {
        _model = model;
    }
...
- (void)commitButtonAction:(UIButton *)button {
    XYRepaymentController *vc = [[XYRepaymentController alloc] init];
