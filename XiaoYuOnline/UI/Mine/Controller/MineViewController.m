...
@interface MineViewController ()<XYMineTableHeaderViewDelegate> {
    BOOL _isLogin;
}
...
- (void)dealloc {
    [[NSNotificationCenter defaultCenter] removeObserver:self name:OLUserDidLoginNotification object:nil];
    [[NSNotificationCenter defaultCenter] removeObserver:self name:OLUserDidLogoutNotification object:nil];
    [[NSNotificationCenter defaultCenter] removeObserver:self name:OLUserAssetDidChangedNotification object:nil];
}
...
- (void)viewDidLoad {
    ...
    [self loadUIContent];
    // 监听用户状态
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(userDidLogin) name:OLUserDidLoginNotification object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(userDidSignOut) name:OLUserDidLogoutNotification object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(userAssetChanged:) name:OLUserAssetDidChangedNotification object:nil];
}
...
- (void)loadUIContent {
    if (XYCurrentUser.userState == OLUserStateLogin) {
        ...
        [self sendRequest:^(XMRequest * _Nonnull request) {
            ...
            OLUserAsset *asset = [XYCurrentUser updateUserAssetWithApiData:responseObject];
            [weakSelf updateTableHeaderViewWiteAsset:asset];
        }];
    }
}

- (void)updateTableHeaderViewWiteAsset:(OLUserAsset *)asset {
