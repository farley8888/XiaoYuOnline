...
@interface ScoreShopViewController ()<UITableViewDelegate, UITableViewDataSource>
...
- (void)loadTableDataRefresh:(BOOL)isRefresh {
    ...
    [self sendSilentRequest:^(XMRequest * _Nonnull request) {
        ...
        for (NSDictionary *data in responseObject[@"cash"][@"pro"]) {
            ScoreModel *model = [[ScoreModel alloc] initWithScoreShopListData:data];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        ScoreModel *model = _scoreList[indexPath.row];
