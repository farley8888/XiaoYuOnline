...
@implementation PhoneCodeVerifyView
...
+ (PhoneCodeVerifyView *)phoneCodeVerifyView {
    UIWindow *window = [[[UIApplication sharedApplication] delegate] window];
    UIView *tagView = [window viewWithTag:kXYPhoneCodeVerifyViewTag];
    if (tagView) {
        return (PhoneCodeVerifyView *)tagView;
    } else {
        for (UIView *v in window.subviews) {
            if ([v isKindOfClass:[PhoneCodeVerifyView class]]) {
                v.tag = kXYPhoneCodeVerifyViewTag;
                return (PhoneCodeVerifyView *)v;
            }
        }
    }
    
    PhoneCodeVerifyView *view = [[PhoneCodeVerifyView alloc] init];
