...
#import "HomeSignView+Custeffet.h"
#import "UIView+Common.h"
#import "FSCalendar.h"

static const NSUInteger kSignViewTag = 1200;

@interface HomeSignView ()<FSCalendarDelegate, FSCalendarDataSource>
...
+ (HomeSignView *)signView {
    ...
    if (tagView) {
        return (HomeSignView *)tagView;
    } else {
        for (UIView *v in window.subviews) {
            if ([v isKindOfClass:[HomeSignView class]]) {
                v.tag = kSignViewTag;
                return (HomeSignView *)v;
            }
        }
    }
    
    HomeSignView *view = [[HomeSignView alloc] init];
