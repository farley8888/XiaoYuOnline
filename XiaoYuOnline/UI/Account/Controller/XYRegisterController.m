...
@interface XYRegisterController ()<UITextFieldDelegate>
@property (nonatomic, strong) NSArray<XYRegisterModel *> *itemList;
...
- (void)viewDidLoad {
    ...
    _itemList = [XYRegisterModel registerDatasourceList];
    
    self.tableView.rowHeight = 48.0;
    self.tableView.sectionHeaderHeight = 8;
    self.tableView.tableFooterView = [[XYRegisterFooterView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, 160)];
    ...
    [self.tableView registerClass:[XYRegisterViewCell class] forCellReuseIdentifier:registerImageCellReuseID];
...
- (void)addButtonAction {
    XYRegisterFooterView *view = (XYRegisterFooterView *)self.tableView.tableFooterView;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    XYRegisterModel *model = _itemList[indexPath.row];
    /*
    if (indexPath.row == 1) {
...
- (void)sendVerifyCodeButtonAction:(UIButton *)button {
    ...
    [self sendRequest:^(XMRequest * _Nonnull request) {
        ...
        if ([view isKindOfClass:[XYSendVerifyCodeView class]]) {
            [(XYSendVerifyCodeView *)view start60SecondCountDown];
        }
